...
class zeta {
	public:
		...
		void Select_Mode(byte Mode);				// Device sleep, Ready, RX mode(see data sheet for values) 
		void RX_Mode(byte Ch, byte PLength);		// enter RX mode select channel and packet length 
		void Set_RF_Baud(byte Baud);				// see data sheet for values 
		void Sync_Byte(byte Sync1, byte Sync2, byte Sync3, byte Sync4); // change sync bytes on rf zeta 
		
		void Send_Packet(byte Ch, byte PLength);	// send packet opening 
